...
do
	...
	RMSD=$(TMscore -c -ter -3 $pdb_filtered/$FILE_2 relaxed_output/${FILE_2::-4}*.pdb | grep RMSD | tr -d -c '[. [:digit:]]' | xargs -n1)
...
do
	...
	#Confidence is > 0.86 instead of 0.8, because the multiple chain target is inflating the iptm
	if [[ $(echo "$confidance > 0.86" | bc -l) -eq 1 && $(echo "$RMSD < 1.5" | bc -l) -eq 1 ]]
	then
		echo $LINE 
	fi
done 
