...
then
        ...
        You need pdb_rplresname.py from pdb-tools in your working directory"
else
...
#put thing to make HIS to HIP (unzip_output)
for FILE in $(ls unzip_output/*.pdb)
do
...
mpiexec -np $n_cores rosetta_scripts.mpi.linuxgccrelease -parser:protocol relax.xml -beta_nov16 -out:path:pdb relaxed_output -in:file:l list.txt -extra_res_fa HIS_P.params

rm score.sc list.txt
#put thing to make HIP to HIS (unzip_output)
for FILE in $(ls unzip_output/*.pdb)
do
...
#put thing to make HIP to HIS (relaxed_output)
for FILE in $(ls relaxed_output/*.pdb)
do
        python pdb_rplresname.py -HIP:HIS $FILE > ${FILE::-4}"_temp.pdb"
        mv ${FILE::-4}"_temp.pdb" $FILE
done
